...
class MarketConfigSet:
    class Builder:
        ...
        def add_stocks(self, stocks: List[str], from_dt: date, untill_dt: date) -> MarketConfigSet.Builder:
            self.data.append(MarketConfig(stocks.copy(), from_dt, untill_dt))
            return self

        def add_stock(self, stock: str, from_dt: date, untill_dt: date) -> MarketConfigSet.Builder:
            self.data.append(MarketConfig([stock], from_dt, untill_dt))
            return self

        def add_stocks_set(self, stocks_set: List[List[str]], from_dt: date, untill_dt: date) -> MarketConfigSet.Builder:
