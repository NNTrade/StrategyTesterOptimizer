...
class OptimizationConfig:
    ...
    def candle_ds_cfg(self) -> CandleDataSetConfig:
        """candle data set config using in optimization process.
        """
        return self.__cds_cfg

    @property
    def period(self) -> DatePeriod:
        """using period in optimiaztion process
        """
        return self.__period    

    def split(self, chunks_count: int) -> List[OptimizationConfig]:        
        return [OptimizationConfig(self.candle_ds_cfg, p, self.strategy_cfg_set) for p in self.period.split(chunks_count)]
