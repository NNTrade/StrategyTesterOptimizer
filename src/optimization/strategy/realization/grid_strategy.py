...
from ..abs_strategy import absStrategy,StrategyConfigSet, StrategyConfig,SimulationReport,absReportComparer
...
class GridStrategy(absStrategy):    
  def __init__(self,strategy_config_set:StrategyConfigSet, run_report_comparer:absReportComparer) -> None:
    super().__init__(GridStrategy, strategy_config_set, run_report_comparer)
  ...
  def first(self)->StrategyConfig:
    ...
    self._logger.debug(f"Get first strategy config:\n{first_cfg}")
    return first_cfg
  
  def _get_next(self,prev_run_report:SimulationReport)->StrategyConfig|None:
    try:
      next_cfg = self.__avaliable_str_cfg.pop()
      self._logger.debug(f"Get next strategy config:\n{next_cfg}")
      return next_cfg
    except IndexError:
      return None
