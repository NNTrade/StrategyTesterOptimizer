...
class DealMetric_avg_net_profit_by_all_success_loss_TestCase(unittest.TestCase):
    ...
    def test_WHEN_pos_deals_THEN_correct(self):
        ...
        deals:List[Deal] = [
            ...
            Deal(datetime(2023, 9, 1), open_price=1, asset="A", amount=1,
                 using_capital=6).close_deal(datetime(2023, 9, 2), 4),  # +3/6
            Deal(datetime(2023, 9, 2), open_price=4, asset="A", amount=3,
                 using_capital=6).close_deal(datetime(2023, 9, 3), 6),  # +6/6
            Deal(datetime(2023, 9, 2), open_price=3, asset="A", amount=3,
                 # 0
                 using_capital=10).close_deal(date=datetime(2023, 9, 3), price=3),
            Deal(datetime(2023, 9, 1), open_price=2, asset="A",
                 amount=3, using_capital=12).set_last_price(4),  # +6/12
        ]
        ...
        # Assert
        self.assertAlmostEqual((3+6+6)/4, asserted_avg_net_profit) # type: ignore
    ...
    def test_WHEN_neg_deals_THEN_correct(self):
        # Array
        deals: List[Deal] = [
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, using_capital=6,
                 asset="A").close_deal(datetime(2023, 9, 2), 1),  # -3/6
            Deal(datetime(2023, 9, 2), open_price=6, amount=3, using_capital=6,
                 asset="A").close_deal(datetime(2023, 9, 3), 4),  # -6/6
            Deal(datetime(2023, 9, 2), open_price=3, amount=3, using_capital=10,
                 asset="A").close_deal(datetime(2023, 9, 3), 3),  # 0
            Deal(datetime(2023, 9, 1), open_price=4, amount=3,
                 using_capital=12, asset="A").set_last_price(2),  # -6/12
        ]
        assert deals[0].profit == -3
        assert deals[1].profit == -6
        assert deals[2].profit == 0
    ...
    def test_WHEN_mixed_deals_THEN_correct(self):
        ...
        deals: List[Deal] = [
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 2), 1),  # -3
            Deal(datetime(2023, 9, 2), open_price=4, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 6),  # +6
            Deal(datetime(2023, 9, 2), open_price=3, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 3),  # 0
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, asset="A",
                 using_capital=10).set_last_price(4),  # +6
        ]
        assert deals[0].profit == -3
        assert deals[1].profit == 6
        assert deals[2].profit == 0
...
class DealMetric_avg_net_interest_by_all_success_loss_TestCase(unittest.TestCase):
    ...
    def test_WHEN_no_deal_THEN_0(self):
        ...
        # Act
        asserted_avg_net_profit = asserted_rep.avg_interest_all
        asserted_avg_net_income = asserted_rep.avg_interest_by_success
        asserted_avg_net_loss = asserted_rep.avg_interest_by_loss
    ...
    def test_WHEN_pos_deals_THEN_correct(self):
        ...
        assert deals[0].interest == 3/6
        assert deals[1].interest == 6/6
        assert deals[2].interest == 0/6
        assert deals[3].interest == 6/12
        ...
        # Assert
        self.assertAlmostEqual((3/6+6/6+6/12)/4, asserted_avg_net_profit) # type: ignore
        self.assertAlmostEqual((3/6+6/6+6/12)/3, asserted_avg_net_income)
        self.assertAlmostEqual(0, asserted_avg_net_loss)
    ...
    def test_WHEN_neg_deals_THEN_correct(self):
        ...
        # Assert
        self.assertAlmostEqual((-0.5-1-0.5)/4, asserted_avg_net_profit) # type: ignore
        self.assertAlmostEqual(0, asserted_avg_net_income)
        self.assertAlmostEqual((-0.5-1-0.5)/3, asserted_avg_net_loss)

    def test_WHEN_mixed_deals_THEN_correct(self):
        # Array
        deals: List[Deal] = [
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 2), 1),  # -3/10
            Deal(datetime(2023, 9, 2), open_price=4, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 6),  # +6/10
            Deal(datetime(2023, 9, 2), open_price=3, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 3),  # 0/10
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, asset="A",
                 using_capital=10).set_last_price(4),  # +6/10
        ]
        assert deals[0].interest == -3/10
        assert deals[1].interest == 6/10
        assert deals[2].interest == 0
        assert deals[3].interest == 6/10
        asserted_rep = DealMetric(deals)

        # Act
        asserted_avg_net_interest = asserted_rep.avg_interest_all
        asserted_avg_net_income = asserted_rep.avg_interest_by_success
        asserted_avg_net_loss = asserted_rep.avg_interest_by_loss

        # Assert
        self.assertAlmostEqual(
            (-3/10+6/10+6/10)/4, asserted_avg_net_interest)  # type: ignore
        self.assertAlmostEqual((6/10+6/10)/2, asserted_avg_net_income)  # type: ignore
        self.assertAlmostEqual((-3/10)/1, asserted_avg_net_loss)  # type: ignore
...
class DealMetric_PROM_TestCase(unittest.TestCase):
    ...
    def test_WHEN_request_THEN_correct(self):
        ...
        deals: List[Deal] = [
            ...
            Deal(datetime(2023, 9, 1), open_price=2, amount=3,
                 asset="A", using_capital=10).set_last_price(4),  # +6
        ]
        assert deals[0].profit == -3
        assert deals[1].profit == 6
        assert deals[2].profit == 0
    ...
    def test_WHEN_only_success_THEN_correct(self):
        ...
        deals: List[Deal] = [
            Deal(datetime(2023, 9, 1), open_price=1, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 2), 2),  # +3
            Deal(datetime(2023, 9, 2), open_price=4, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 6),  # +6
            Deal(datetime(2023, 9, 2), open_price=3, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 3),  # 0
    ...
    def test_WHEN_only_fail_THEN_correct(self):
        ...
        deals = [
            ...
            Deal(datetime(2023, 9, 2), open_price=6, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 4),  # -6
            Deal(datetime(2023, 9, 2), open_price=3, amount=3, asset="A",
                 using_capital=10).close_deal(datetime(2023, 9, 3), 3),  # 0
            Deal(datetime(2023, 9, 1), open_price=4, amount=3, asset="A",
                 using_capital=10).set_last_price(2),  # -6
        ]
        asserted_rep = DealMetric(deals)
        expected_AGP = 0
    ...
    def test_WHEN_all_deals_is_zero_THEN_None(self):
        ...
        deals = [
            Deal(datetime(2023, 9, 2), open_price=3,  amount=3, using_capital=10,
                 asset="A").close_deal(datetime(2023, 9, 3), 3),  # 0
            Deal(datetime(2023, 9, 1), open_price=2, amount=3, using_capital=10,
